...
// TOC chip strip. Heading IDs are auto-generated by Nuxt Content's
// rehype-slug pass (which uses github-slugger). We don't try to force
// stable IDs across locales — Nuxt Content / mdc doesn't support the
// `## Heading {#id}` Pandoc syntax (it falls through as literal text
// in the rendered H2). Instead, we mirror the slugger's output per
// locale here. Verified slugs against `new GithubSlugger().slug(text)`
// for each H2 in content/guide.md and content/is/guide.md.
//
// `weather--cloud-cover` is double-hyphened by design: github-slugger
// strips the `&` and leaves both adjacent spaces as hyphens. If you
// rename that heading to "Weather and Cloud Cover", update both the
// EN slug here and the IS heading text.
const TOC_IDS: Record<string, Record<string, string>> = {
  en: {
    whats_happening:      'whats-happening',
    the_path_of_totality: 'the-path-of-totality',
    best_viewing_spots:   'best-viewing-spots',
    weather_cloud_cover:  'weather--cloud-cover',
    getting_there:        'getting-there',
    what_to_bring:        'what-to-bring',
    eclipse_day_timeline: 'eclipse-day-timeline',
    faq:                  'faq',
  },
  is: {
    whats_happening:      'hvað-er-að-gerast',
    the_path_of_totality: 'leið-heilmyrkvans',
    best_viewing_spots:   'bestu-áhorfsstaðirnir',
    weather_cloud_cover:  'veður-og-skýjahula',
    getting_there:        'að-komast-þangað',
    what_to_bring:        'hvað-á-að-taka-með',
    eclipse_day_timeline: 'tímalína-sólmyrkvadagsins',
    faq:                  'algengar-spurningar',
  },
}

// The English map is the canonical key set; IS mirrors the same keys
// with locale-specific slugs. Drives both the chip order and the
// fallback when a future locale lacks an entry here.
const TOC_KEYS = Object.keys(TOC_IDS.en!)

const toc = computed(() => {
  const ids = TOC_IDS[String(locale.value)] ?? TOC_IDS.en!
  return TOC_KEYS.map(key => ({
    id: ids[key]!,
    label: t(`guide.toc.${key}`),
  }))
})
